...
class SleepUi {
public:
  void showCloud(bool connected) {
  int newState = connected ? 1 : 0;

  // Do not redraw if the state did not change
  if (newState == _lastCloudState) {
    return;
  }

  _lastCloudState = newState;

  const int x = 204;
  const int y = 3;

  // Clear the old icon
  _tft.fillRect(200, 1, 39, 22, C_HEADER);

  uint16_t cloudColor = connected ? C_OK : C_DIM;

  // Draw cloud shape
  _tft.fillCircle(x + 7,  y + 10, 5, cloudColor);
  _tft.fillCircle(x + 14, y + 7, 7, cloudColor);
  _tft.fillCircle(x + 22, y + 10, 5, cloudColor);
  _tft.fillRoundRect(x + 4, y + 10, 23, 8, 4, cloudColor);

  // Red slash means disconnected
  if (!connected) {
    _tft.drawLine(
      x + 3,
      y + 2,
